$( document ).ready(function() {
  ...
  function reloadReplacement() {
    ...
    $('section').each(function (index, value) {
      ...
      backgroundURL = backgroundURL.replace(/.*\s?url\([\'\"]?/, '').replace(/[\'\"]?\).*/, '');
      // Get URL from Background-Image CSS Property
      backgroundImageURL = backgroundImageURL.replace(/.*\s?url\([\'\"]?/, '').replace(/[\'\"]?\).*/, '');
      ...
      if (backgroundURL.match("^file:///")) {
        ...
        // Replace ^media_src^
        backgroundURL = backgroundURL.replace(thisPath+'/css/', '').replace('%5E', '').replace('%5E', '').replace('media_src_', '/img/');

        // Set image file
        $(this).css({'background-image': 'url('+thisPath + backgroundURL+')'});
      }
      ...
      if (backgroundImageURL.match("^file:///")) {
        ...
        // Replace ^media_src^
        backgroundImageURL = backgroundImageURL.replace(thisPath+'/css/', '').replace('%5E', '').replace('%5E', '').replace('media_src_', '/img/');

        // Reset property
        $(this).removeAttr('style');
        // Set image file
        $(this).css({'background-image': 'url('+thisPath + backgroundImageURL+')'});
      }
    ...
    $('img').each(function (index, value) {
      ...
      if (imageSrc.indexOf('^media_src_') === 0) {
        // Replace ^media_src^
        imageSrc = imageSrc.replace('^', '').replace('^', '').replace('media_src_', '/img/');
        // Set image file
